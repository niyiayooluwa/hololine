import 'package:dart_either/dart_either.dart';
import 'package:hololine_flutter/core/errors/failures.dart';
import 'package:hololine_flutter/module/auth/data/repository/auth_repository_impl.dart';
import 'package:hololine_flutter/module/auth/data/repository/auth_repository.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';

class ResetPasswordRequestUsecase {
  final AuthRepository _authRepository;

  ResetPasswordRequestUsecase(this._authRepository);

  Future<Either<Failure, bool>> call(String email) async {
    if (email.isEmpty) {
      return Left(AuthFailure.invalidEmail());
    }

    if (!RegExp(
            r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9]+\.[a-zA-Z]+")
        .hasMatch(email)) {
      return Left(AuthFailure.invalidEmail());
    }

    return await _authRepository.initiatePasswordReset(email);
  }
}

final resetPasswordRequestUsecaseProvider = Provider<ResetPasswordRequestUsecase>((ref) {
  final authRepository = ref.watch(authRepositoryProvider);
  return ResetPasswordRequestUsecase(authRepository);
});
